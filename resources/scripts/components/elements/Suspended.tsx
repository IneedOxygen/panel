    ...
    <>
      ...
      <PageContentBlock title={"Server Suspended"}>
        ...
        <div css={tw`flex justify-center`}>
          <div css={tw`w-full sm:w-3/4 md:w-1/2 p-12 md:p-20 bg-neutral-900 rounded-lg shadow-lg text-center relative`}>
            ...
            {renewable ? (
              <>
                <p css={tw`text-sm my-2`}>
                  Your server has been suspended due to it not being renewed on time. Please click the &apos;Renew&apos;
                  button in order to reactivate your server. If you want to delete your server, the resources will
                  automatically be added back to your account so you can re-deploy a new server easily.
                </p>
                <Button className={"mx-2 my-1"} onClick={() => setRenewDialog(true)} disabled={isSubmit}>
                  Renew Now
                </Button>
                <Button.Danger className={"mx-2 my-1"} onClick={() => setDeleteDialog(true)} disabled={isSubmit}>
                  Delete Server
                </Button.Danger>
              </>
            ) : (
