    ...
    >
      <Container className={"j-up lg:grid lg:grid-cols-3 my-10"}>
        <ContentBox title={"Your Referral Codes"} css={tw`sm:mt-0`}>
          ...
          {codes.length === 0 ? (
            ...
            codes.map((code, index) => (
              <GreyRowBox key={code.code} css={[tw`bg-neutral-900 flex items-center`, index > 0 && tw`mt-2`]}>
        ...
        <ContentBox title={"Available Perks"} css={tw`mt-8 sm:mt-0 sm:ml-8`}>
          <h1 css={tw`text-xl`}>
            You will recieve <span className={"text-green-500"}>{reward}</span> credits for every user you refer to this
            Panel.
          </h1>
        </ContentBox>
        <ContentBox title={"Users Referred"} css={tw`mt-8 sm:mt-0 sm:ml-8`}>
          ...
          {activity.length === 0 ? (
            ...
            activity.map((act, index) => (
              <GreyRowBox key={act.code} css={[tw`bg-neutral-900 flex items-center`, index > 0 && tw`mt-2`]}>
                <Icon.GitBranch css={tw`text-neutral-300`} />
                <div css={tw`ml-4 flex-1 overflow-hidden`}>
