    ...
    <PageContentBlock title={"Dashboard"} css={tw`mt-4 sm:mt-10`} showFlashKey={"dashboard"}>
      ...
      {rootAdmin && (
        <div css={tw`mb-10 flex justify-between items-center`}>
          <div>
            <h1 className={"j-left text-5xl"}>{showOnlyAdmin ? "Showing others' servers" : "Showing your servers"}</h1>
            <h3 className={"j-left text-2xl mt-2 text-neutral-500"}>Select a server to view, update or modify.</h3>
          </div>
          <Switch
          ...
          {({ items }) =>
            items.length > 0 ? (
              <div className={"lg:grid lg:grid-cols-3 gap-4"}>
                <>
                  {items.map((server) => (
                    <ServerRow key={server.uuid} server={server} className={"j-up"} css={tw`mt-2`} />
                  ))}
                </>
              </div>
            ) : (
